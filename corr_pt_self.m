function corr_pt_self(pt_series_file,ix,output_file,FD_mask)

%% corr_pt_dt(pt_series_file,dt_series_file,ix,output_file)
%% originally written by Oscar Miranda-Dominguez, OCt 13, 2016
%% Edited by Moosa Ahmed, Sept 28, 2017
%  (converting pt_dt to pt_self
...
%%
addpath('J:\code\internal\utilities\corr_pt_dt/support_files');
...
if isempty(output_file)
    [pathstr_dt,name_dt,ext_dt] = fileparts(pt_series_file);
    % pathstr_out=pathstr_pt;
    output_file=[pathstr_dt filesep 'corr_pt_self'];
end
...
[N_rois, N_TR]=size(pt_series);% count the total rois in this parcellation
...
%% Do motion censoring
pt_series_final=pt_series_final(:,FD_mask);
dt_series=dt_series(:,FD_mask);

%% calculate correlations
R=corr(pt_series_final',dt_series');
%newcii.cdata=R';
%% Save data as cifti
output_file=[output_file '.csv'];
%ciftisave(newcii,output_file,path_wb_c); % Making your cifti
csvwrite(output_file,R)
%% Validation
% temp=randi(3);
% ix1=ix(temp);
% ix2=randi(91282);
